...
| 11 | THE DAY RELEASE: Calendar as default landing + day-wise scheduling | PLANNED | v3.2.0/vc11. Added 2026-08-18 on owner request ("calendar view as default view on landing to allow manage day wise schedule"). Month grid + day pane aggregating every dated item the app already knows (event reminders, contact-due, occasions, campaigns, open follow-ups, logged interactions) PLUS a new device-local `state.scheduledItems` collection so a volunteer can plan a visit/call/meeting/task on any day, mark it done (which logs an Interaction), reschedule or delete. Also closes a real gap: `Interaction.followUpDate` has no completion state today, so follow-ups stay due forever — additive `followUpCompletedAt` added. Calendar takes the home slot (`Dashboard` nav tab → `Calendar`, `setDefaultRoute('/calendar')`); **My Day is retained** at `#/dashboard`, linked from the day pane, so the change is one-line revertible. Time-of-day slots + volunteer assignment explicitly OUT of v1 (assignment would force scheduled items through sync merge — separate design problem). Requires new date foundation: `Reminder`/`normalizeEventDate` cannot represent past dates and `getRemindersForMonth` ignores year, so a year-aware `CalendarService` derivation is built on a shared leap-clamped `resolveAnnualDate()`; also fixes a pre-existing UTC off-by-one in `_generateFrequencyReminder`. **Detailed sequenced tasks: `ITERATION_11_PLAN.md`.** **Scope extended 2026-08-20 (owner request: "perfect deliverable … robust, correct, easy to use … helping them help more people"):** the iteration now ships as a *release deliverable*, not just a feature. Added **Phase R (safe delivery)** — a read-only audit of the upgrade path found four pre-existing defects that would bite on any reinstall: `generateFullBackup` stamps `backupType:'full'` but omits `occasions`/`campaigns` (and would omit `scheduledItems`); a full-backup *file* cannot be restored because the file route always goes through `SyncService.merge`, which reads only 3 collections; `android/` has **no `signingConfig` at all**, so the only installable artifact is a debug APK whose key (`~/.android/debug.keystore`, SHA-1 `70:E9:6A:21:…`) is an un-backed-up single point of failure; and CI's `assembleDebug` regenerates that keystore per run, so a CI APK forces `INSTALL_FAILED_UPDATE_INCOMPATIBLE` → uninstall → total data loss. Added **Phase U (volunteer usability)** — what's-new card, two coach marks, a backup-age nudge derived from the existing `syncLog`, Marathi-first strings, colour-blind-safe overdue marking. Phase E rewritten around an **upgrade rehearsal** (install v3.1.0 with real data → upgrade in place → verify) plus a **restore drill** on a spare device, as the release acceptance test; test target raised 165 → 230. Decision under delegated ownership: v3.2.0 ships on the **existing debug key** for a zero-uninstall upgrade; release signing is wired now but adopted at v4.0, after Phase R0 is proven on hardware. **Scope extended again 2026-08-20 (same session):** **Phase S — shareable plans.** Owner corrected the device-local assumption: these NGOs operate entirely by passing backups as WhatsApp messages, some with no root machine at all, so a plan made at the office must reach the phone that executes it. Approved with the open-ended export menu **rejected** — three exports named by the job (Send everything / Send plans / Share this day), justified by measurement against the real `TextSyncService` pipeline: a full backup of 25 visitors + 200 interactions is **5 WhatsApp messages**, a week of plans is **1**, and the gap widens as history grows. Adds five merge rules the proposal did not cover — sending-is-assigning (no assignee field), plans carry a denormalised `visitorName` + visitor stubs, cancellations travel as 30-day tombstones, `done` is terminal on the receiving device, and idempotent/stale re-import. **Phase F — native file export/import.** Owner reported files failing on mobile; diagnosed as a bug, not a platform limit: `saveFile()` fails all three paths on Android (no Web Share API in WebView, `@capacitor/share` never installed, no `setDownloadListener`, blob URLs unusable by DownloadManager) **and reports success regardless** — logged as F-DEFECT-0, the highest-severity defect found in the codebase, because it manufactures false confidence in a backup that was never written. Fix adds `@capacitor/filesystem` + `@capacitor/share` at runtime via `window.Capacitor.Plugins` (web bundle and single-file `file://` build untouched), plus an intent-filter so a backup can be imported by tapping it in WhatsApp. Governing rule: the text route stays the **guaranteed** path and Phase F is strictly additive. Release scope +4 to +6 days; bundled rather than split because a remote WhatsApp rollout is expensive and the dev team owns it. **Scope extended a third time 2026-08-20:** **Phase V — inbound visit intake.** Owner supplied three rough phone-call scenarios for analysis; the domain reading is that supporters come **to** the NGO on personally significant dates, so `visit` was ambiguous and B/C had built the outbound one. Owner confirmed **inbound is the primary flow (G10-R supersedes G10)** — day pane now leads with 'Coming to us'. Adds phone-first identity resolution, occasion capture storing the occasion's own date separately from the visit date, third-party occasions creating Contact+event so the existing reminder engine carries them forward, happened/did-not-happen outcome (auto-complete would thank people for visits that never occurred), and an annual return-reminder replacing the owner's two-branch 'last visit date' rule with one invariant + one dedup rule. Phone is mandatory as **phone-first** with a null-storing escape — a hard-required field makes staff type `0000000000`, which under last-10-digit dedup merges unrelated supporters and propagates the merge via sync. Donation tracking explicitly OUT. **Phase O — occasion greetings.** Verified first: 15 Aug already ships as a seeded built-in and Foundation Day already works as a fixed-date custom occasion. Real defect found instead — **no seeded occasion is a festival** and `Occasion` holds only month/day, so lunar-calendar festivals fire on the wrong day every year after the first, while two code comments tell users movable festivals are supported. Fix: optional per-year date table, hard **never-guess** rule, visible 'needs a date' state, no lunar library, and festival dates almanac-sourced and owner-confirmed rather than recalled. |
| 11.5 | REACH TAIL: B3 post-comm auto-log polish + F-Contacts phone-contact import | PLANNED | v3.2.1/vc12. Tail of the Reach release (renumbered 10.5→11.5 on 2026-08-18 when the Calendar iteration was inserted at 11); see `ITERATION_10_PLAN.md`. |
| 12 | THE FIELD RELEASE: Visit Planner by City (lane D2) | PLANNED | v3.3.0/vc13 (renumbered 11→12). Group active visitors by city, per-city field-round view (upcoming events + lapsed), mark-visited → visit Interaction. Offline, no map tiles. Task sketch in `ITERATION_10_PLAN.md`. |
| 13 | THE POLISH RELEASE: Design-language elevation (UX-6 screen set) | PLANNED | v3.4.0/vc14 (renumbered 12→13). Driven by `frontend-design`; coherent tokens, Devanagari type scale, polished states, a11y. Task sketch in `ITERATION_10_PLAN.md`. |
